...
import generalPurePursuit as LOGIC
import lidarPerception as LIDAR
import ackermanCalculator as ACK
import rclpy
...
testLocationStr = "./wp_optimal_OO.csv"
...
class coreNode(Node): 
    ...
    for wp in tm._compute_speed_profile():
        map.append(wp)
    
    pp = LOGIC.generalPurePursuit(0.33)
    LP = LIDAR.lidarPerceptiom(0.33,)
    i=0
    for wp in map:
        pp.computeCurvature(wp.x,
                            wp.y,
                            wp.theta,
                            map[i+1 if i+1 != len(map) else i].x > wp.x,
                            map[i+1 if i+1 != len(map) else i].y > wp.y)
        i += 1
